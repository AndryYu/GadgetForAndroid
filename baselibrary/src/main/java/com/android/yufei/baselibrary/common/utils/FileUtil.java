...
public class FileUtil {
    ...
    public static void writeLogCrashContent(Context context, final String message) {
        mContext = context;
        new Thread(new Runnable() {
            @Override
            public void run() {
                FileUtil.writeFile(getFileDir("Log") + "/crash.log", TimeUtil.getCurTimeString() + " : " + message, true);
            }
        }).start();
    }
